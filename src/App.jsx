...
function App() {
  ...
  const  renderTasks = () => tasks.map((task,index)=>{
    for (index; index <  tasks.length ; index++) {
      return <Task key={index} id={index} detail={task} tasks={tasks} deleteTask={deleteTask}></Task>}
    })
  return (
    <>
      <h1 className='title '>To Do List </h1>
      <form >
          <input type="text" placeholder='Add a task...' onChange={(e) => handleChange(e)} onKeyDown={(e) => handleKeyDown(e)}value={task}/>
        </form>
